#ifndef LCD1602_H_
#define LCD1602_H_

#include "main.h"

// ------- Выбрать и настроить порт: ------- 

#define	SETPORT	(DDRA=0xFF)
#define	MODEPORT (PORTA=0x00)

#define E1 (PORTA|=(1<<3)) // установка линии E в 1 стробирующая шина
#define E0 (PORTA &=~(1<<3))// установка линии E в 0
#define RS1 (PORTA|=(1<<2)) // установка линии RS в 1 (данные)
#define RS0 (PORTA &=~(1<<2)) // установка линии RS в 0 (команда)

// ------- Конец настроек порта ------- 

void LCD1602_ini(void);
void send_lcd_char(uint8_t c); // отправка 1 символа
void set_lcd_pos(uint8_t x, uint8_t y); // установка позиции Х 0-15, У 0-1.
void send_lcd_string(uint8_t *str);// отправка строки по указателю массива
void create_symb(uint8_t index_symb, const uint8_t *data); //пользовательский символ


#endif /* LCD1602_H_ */
